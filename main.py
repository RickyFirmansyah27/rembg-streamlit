...
if uploaded_file is not None:
    ...
    with col2:     
        ...
        filter = st.sidebar.radio('Covert your photo to:', ['Original','Gray Image','Black and White', 'Delete Background', 'Pencil Sketch', 'Blue Background', 'Blur Effect'])
        if filter == 'Gray Image':
                ...
                img = np.zeros(input.shape, dtype=np.uint8)
                output = remove(input)
                converted_img = np.array(output.convert('RGB'))
